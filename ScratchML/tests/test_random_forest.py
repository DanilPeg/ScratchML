...
from ScratchML.ensemble.random_forest import RandomForestClassifier, RandomForestRegressor
...
for i, (model, name) in enumerate([
    ...
    (RandomForestClassifier(n_estimators=30, max_features=1, random_state=42, tree_params={"max_depth":4, "min_samples_leaf":5}), "RandomForest")
]):
...
for i, (model, name) in enumerate([
    ...
    (RandomForestRegressor(n_estimators=30, max_features=2, random_state=42, tree_params={"max_depth":10, "min_samples_leaf":5}), "RandomForest")
]):
    model.fit(X_train, y_train)
...
for i, (model, name) in enumerate([
    ...
    (RandomForestRegressor(n_estimators=30, max_features=1, random_state=42, tree_params={"max_depth":6, "min_samples_leaf":2}), "RandomForest")
]):
    model.fit(X_train, y_train)
    y_pred = model.predict(X_test)
    plt.subplot(1, 2, i+1)
